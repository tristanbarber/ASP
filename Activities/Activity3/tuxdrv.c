...
{
	pr_info(" Tristan Barber: OPENING device: %s:\n\n", MYDEV_NAME);
	return 0;
}

static int mycdrv_release(struct inode *inode, struct file *file)
{
	pr_info(" Tristan Barber: CLOSING device: %s:\n\n", MYDEV_NAME);
	return 0;
}
...
mycdrv_read(struct file *file, char __user * buf, size_t lbuf, loff_t * ppos)
{
	...
	*ppos += nbytes;
	pr_info("\n Tristan Barber: READING function, nbytes=%d, pos=%d\n", nbytes, (int)*ppos);
	return nbytes;
}
	     ...
	     loff_t * ppos)
{
	...
	*ppos += nbytes;
	pr_info("\n Tristan Barber: WRITING function, nbytes=%d, pos=%d\n", nbytes, (int)*ppos);
	return nbytes;
}
